    ...
    fn read_atom(&mut self) -> Result<Expr> {
        ...
        if let Ok(n) = tok.parse::<i32>() {
            ...
            match tok {
                ...
                "defmacro!" => Ok (Expr::Special(SpecialForm::DefMacro)),
                ...
                "let*" => Ok (Expr::Special(SpecialForm::Let)),
                "quote" => Ok (Expr::Special(SpecialForm::Quote)),
                "quasiquote" => Ok (Expr::Special(SpecialForm::Quasiquote)),
                "macroexpand" => Ok (Expr::Special(SpecialForm::MacroExpand)),
                "catch*" => Ok (Expr::Special(SpecialForm::Catch)),
                "try*" => Ok (Expr::Special(SpecialForm::Try)),
